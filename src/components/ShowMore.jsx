...
export default function ShowMore({ data }) {
    ...
    return (
        <div className='md:pt-14 pt-10 md:container flex justify-center items-center'>
            {data.length > 0 ? (<div>
                <button className={`${show ? "hidden" : "flex"} bg-[#E50914] but items-center gap-1 text-white px-3 py-2 rounded`} onClick={() => setShow(true)}><VscDebugRestart className='icon'/>More news</button>                   
                <div>
                    <div className={` flex-wrap ${show ? "flex" : "hidden"}`}>
                        {data?.map((item,idx) => (
                            <Link key={idx} className='md:w-1/2 w-full p-5' to={`/products/${item.id}`}>
                                ...
                                <div className="relative  w-full flex  h-[130px]">
                                    ...
                                    <div className="ml-3 rounded pb-0 flex justify-between flex-col items-start text-[16px] md:text-[18px] font-semibold">
                                        <h2 className='hover:text-[#E50914]'>{item.title}</h2>
